...
try {
    ...
    if(!message.content.toLowerCase().startsWith(prefix)){
        if(message.content ===`<@${message.client.user.id}>`) return message.channel.send(":interrobang: **|** Vous avez oublié mon prefix ? C'est `" + prefix + "` ! Utilisez le avant une commande pour la commande soit prise en compte.\n__Exemple__: `" + prefix + "help`")
    }
